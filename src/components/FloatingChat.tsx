...
const LOGIN_REQUIRED_MESSAGE =
  'Silakan **login** terlebih dahulu untuk bertanya ke Assistant SnapRent. Setelah login, Anda bisa menanyakan harga, daftar produk, dan rekomendasi rental.';

const QUICK_REPLIES = [
  'Ada kamera apa saja?',
  'Kamera buat liburan',
  'Rekomendasi kamera vlog',
  'Kamera Canon yang tersedia',
  'Kamera murah buat pemula',
];
...
const FloatingChat = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!user) {
      setMessages((prev) => [
        ...prev,
        { role: 'model', content: LOGIN_REQUIRED_MESSAGE, loginRequired: true },
      ]);
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
            <div className="flex flex-col gap-1">
              ...
              {mockMode && (
                <span className="text-[10px] font-medium uppercase tracking-wide text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-2 py-0.5 rounded w-fit">
                  MOCK MODE ACTIVE
                </span>
              )}
            </div>
            <button
              type="button"
              onClick={clearChat}
              className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 text-xs underline hover:bg-gray-200 dark:hover:bg-gray-700 px-2 py-1 rounded transition-colors"
            >
              Reset
            </button>
          </div>
                  ...
                  {msg.loginRequired && (
                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300"
                      >
                        Login di sini →
                      </Link>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 space-y-2 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-2 min-w-0">
                        {msg.products.map((p) => (
                          <div
                            key={p.id}
                            className="flex flex-col rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 overflow-hidden min-w-0"
                          >
                            {/* Product Image */}
                            ...
                            <div className="flex flex-col gap-1.5 p-2 flex-1">
                              ...
                              {p.keunggulan && p.keunggulan.length > 0 && (
                                <div>
                                  <p className="text-[10px] font-semibold text-amber-500 flex items-center gap-0.5 mb-0.5">
                                    ☆ Keunggulan:
                                  </p>
                                  <div className="flex flex-wrap gap-1">
                                    {p.keunggulan.slice(0, 3).map((k, i) => (
                                      <span
                                        key={i}
                                        className="px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[9px] border border-gray-200 dark:border-gray-600"
                                      >
                                        {k}
                                      </span>
                                    ))}
                              ...
                              {p.cocokUntuk && p.cocokUntuk.length > 0 && (
                                <div>
                                  <p className="text-[10px] font-semibold text-green-500 flex items-center gap-0.5 mb-0.5">
                                    ◎ Cocok untuk:
                                  </p>
                                  <div className="flex flex-wrap gap-1">
                              ...
                              <div className="flex items-center justify-between mt-auto pt-1 border-t border-gray-100 dark:border-gray-700">
                                <div>
                                  ...
                                  <p className="text-xs font-bold text-amber-600 dark:text-amber-400">
                                    {formatCurrency(p.price)}<span className="font-normal text-[9px] text-gray-400">/hari</span>
                                  </p>
                                </div>
                                <Link
                                  to={`/product/${p.slug}`}
          ...
          >
            ...
            <button
              ...
              className="flex-shrink-0 p-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 text-white transition-colors"
              aria-label="Kirim"
            >
              <Send size={18} />
            </button>
